...
let searchQuery2 = JSON.parse(localStorage.getItem("dietaryRestrictions")) || []
...
if (searchQuery1.length.length > 0 && searchQuery2.length === 0 && searchQuery3.length === 0){
    url = 
    ...
    + searchQuery1
    + "&number=100"
} else if (searchQuery1.length > 0 && searchQuery2.length > 0 && searchQuery3.length === 0) {
    url = 
    "https://api.spoonacular.com/recipes/complexSearch?apiKey="
    + SPOONACULAR_API_KEY
    + "&query="
    ...
    alert("We need some information to get you some recipies(this is a placeholder we might just return to the homepage or not let this happen)")
};
