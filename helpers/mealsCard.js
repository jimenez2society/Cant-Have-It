export function mealsCard(meal) {
  ...
  let item = parser.parseFromString(
    `
    <div  id=${meal.id
    } class="results-item relative  flex bg-whiteFull rounded-[10px] max-w-[890px] h-[230px] overflow-hidden shadow-med">
    ...
    <a class="absolute opacity-0" href=${meal.spoonacularSourceUrl
    } target="_blank "><img src="../assets/svg/link.svg"/></a>
    </div>
        <div class="flex">
        <div href=${meal.spoonacularSourceUrl
      ...
      .map((diet, i) => {
        ...
        console.log(meal.id);
        item = `<span  class="block ${i >= 3 ? "hidden" : ""
      ...
      : ""
              ...
              <span class="text-[8px] ${meal.diets.length < 3 ? "hidden" : "block"
    }">
          ...
          <p class="text-[12px] leading-[19.5px]">
           ${meal.summary.substring(0, 500)}${meal.summary.length >= 300 && "..."
    }
          </p>
        </div>
      </div>`,
